...
flutter-setup:
	...
	@echo "Connecting to 10.171.15.157:$(PORT)..."
	@adb connect 10.171.15.157:$(PORT) 2>&1 | grep -q "connected" || true
	@if adb devices | grep -q "10.171.15.157:$(PORT)\s\+device"; then \
		echo "✅ Connected to 10.171.15.157:$(PORT)"; \
		echo "$(PORT)" > .flutter_device_port; \
		echo "Device port saved to .flutter_device_port"; \
	else \
		echo "❌ Could not connect to 10.171.15.157:$(PORT)"; \
		adb disconnect 10.171.15.157:$(PORT) > /dev/null 2>&1 || true; \
		exit 1; \
	fi
...
flutter-run:
	@if [ -f .flutter_device_port ]; then \
		...
		cd mobile && flutter run -d 10.171.15.157:$$PORT; \
	else \
